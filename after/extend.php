	...
	<table border="1">
		...
		<tbody>
				...
				while ( $data = mysql_fetch_assoc ( $req ) ) {
					...
					echo '<tr><td>' . $data ['client_name'] . '</td><td>' . $clientType . '</td><td>' . $data ['amount'] . '</td><td>' . $data ['duration'] . '</td><td><input name="input'.$i.'" type="text"/></td><td><input type="button" onclick="extendDuration('.$data['distribution_id'].', '.$i.');" value="贷款展期"/></td></tr>';
					$i = $i + 1;
				}
				?>
			</tbody>
